  ...
  const handleCancel = async () => {
    ...
    if (selectedOrder.alternativas_plan_orden && selectedOrder.alternativas_plan_orden.length > 0) {
        ...
        if (alternativesError) {
            console.error('Error al anular alternativas:', alternativesError);
            // No interrumpimos el flujo si falla, pero logueamos el error
        }
    }
  ...
  const handleSaveAndReplaceOrder = async () => {
    try {
        ...
        if (originalIdsToAnnul.length > 0) {
            console.log('Marcando alternativas originales como anuladas:', originalIdsToAnnul);
            ...
            if (annulError) {
                console.error('Error al marcar alternativas como anuladas:', annulError);
                // No lanzamos error para no interrumpir el flujo principal, pero lo registramos
            }
        }
        ...
        try {
            console.log("Generando PDF automático para la nueva orden...");
                ...
                if (altCopy.calendar) {
                    if (Array.isArray(altCopy.calendar)) {
                        ...
                        try {
                            ...
                            console.error('Error al parsear calendar para PDF:', e);
                            parsedCalendar = [];
                        }
                    }
                }
            ...
            console.error("Error al generar PDF automático:", pdfError);
