...
const Alternativas = () => {
  ...
  const mesesCerradosKeySet = useMemo(() => {
    return new Set((mesesCerrados || []).map(r => `${r.anio}-${r.mes}`));
  }, [mesesCerrados]);

  const esMesCerrado = (anioId, mesId) => {
    if (!anioId || !mesId) return false;
    return mesesCerradosKeySet.has(`${anioId}-${mesId}`);
  };

  const fetchMesesCerrados = async () => {
    try {
      ...
      setMesesCerrados(data || []);
    } catch (error) {
      console.error('No se pudieron cargar meses_cerrados:', error);
      setMesesCerrados([]);
    }
  };
  ...
  const handleOpenNuevaAlternativa = () => {
    ...
    if (soloLectura) {
      Swal.fire({
        icon: 'warning',
        title: 'Mes cerrado',
        text: 'Este mes está cerrado. No se pueden crear nuevas alternativas.'
      });
      return;
    }
    cantidadesRef.current = {};
                    ...
                    <TableRow key={alternativa.id}>
                      ...
                      <TableCell>
                        <Box sx={{ display: 'flex', gap: 0.5, alignItems: 'center', flexWrap: 'nowrap' }}>
                          <Tooltip title={soloLectura ? "Mes cerrado (solo lectura)" : (alternativa.numerorden ? "No se puede editar con N° de Orden asignado" : "Editar")}>
                            <span>
                              <IconButton 
                                onClick={() => handleEditAlternativa(alternativa.id)} 
                                disabled={soloLectura || !!alternativa.numerorden}
                                size="small"
                                style={{ 
                                  color: (soloLectura || alternativa.numerorden) ? 'gray' : '#1976d2'
                                }}
                              >
                                <EditIcon />
                              </IconButton>
                          ...
                          <Tooltip title={soloLectura ? "Mes cerrado (solo lectura)" : (alternativa.numerorden ? "No se puede eliminar con N° de Orden asignado" : "Eliminar")}>
                            <span>
                              <IconButton 
                                ...
                                size="small"
                                style={{ 
                                  color: (soloLectura || alternativa.numerorden) ? 'gray' : '#d32f2f'
                                }}
                              >
                                <DeleteIcon />
                              </IconButton>
                          ...
                          <Tooltip title={soloLectura ? "Mes cerrado (solo lectura)" : "Duplicar"}>
                            <IconButton
