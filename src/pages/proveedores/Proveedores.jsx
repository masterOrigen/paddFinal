  ...
  const fetchData = async () => {
    try {
      ...
      const [
        ...
        supabase.from('Proveedores').select(`*, proveedor_soporte:proveedor_soporte(count)`),
      ...
      // Optimización: Carga masiva de relaciones para evitar N+1 queries
      const proveedorIds = proveedoresData.map(p => p.id_proveedor);
      
      // 1. Obtener todas las relaciones proveedor-soporte de una vez
      const { data: allProveedorSoportes } = await supabase
        .from('proveedor_soporte')
        .select('id_proveedor, id_soporte')
        .in('id_proveedor', proveedorIds);
        
      // 2. Obtener todos los IDs de soportes únicos necesarios
      const soporteIds = [...new Set((allProveedorSoportes || [])
        .map(ps => ps.id_soporte)
        .filter(id => id != null))];
        
      // 3. Obtener los detalles de los soportes
      let soportesMap = {};
      if (soporteIds.length > 0) {
        const { data: allSoportes } = await supabase
          .from('Soportes')
          .select('id_soporte, nombreIdentficiador')
          .in('id_soporte', soporteIds);
          
        soportesMap = (allSoportes || []).reduce((acc, soporte) => {
          acc[soporte.id_soporte] = soporte;
          return acc;
        }, {});
      }

      // 4. Asignar soportes a cada proveedor en memoria
      const proveedoresConSoportes = proveedoresData.map(proveedor => {
        const misSoportesIds = (allProveedorSoportes || [])
          .filter(ps => ps.id_proveedor === proveedor.id_proveedor)
          .map(ps => ps.id_soporte);
          
        const misSoportes = misSoportesIds
          .map(id => soportesMap[id])
          .filter(Boolean);
          
        return {
          ...proveedor,
          soportes: misSoportes
        };
      });
