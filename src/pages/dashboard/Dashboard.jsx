...
const Dashboard = () => {
  ...
  const fetchStats = async () => {
    try {
      const [agenciasResult, clientesResult, campanasResult, mediosResult] = await Promise.all([
        ...
        supabase.from('Campania').select('*'),
        supabase.from('Medios').select('*')
      ]);

      setStats({
        agencias: agenciasResult.data?.length || 0,
        clientes: clientesResult.data?.length || 0,
        campanas: campanasResult.data?.length || 0,
        medios: mediosResult.data?.length || 0
      });
    } catch (error) {
  ...
  const fetchClientesConMasCampanas = async () => {
    try {
      ...
      console.error('Error al obtener clientes con más campañas:', error);
    }
  };
