...
const ViewSoporte = () => {
  ...
  const fetchSoporte = async () => {
    ...
    try {
      const [
        { data: soporteData, error: soporteError },
        { data: mediosData, error: mediosError },
        { data: proveedorSoporteData, error: proveedorSoporteError }
      ] = await Promise.all([
        supabase
          .from('Soportes')
          .select('*')
      ...
      if (data) {
        const programasFormateados = data.map(prog => {
          ...
          const validarHora = (hora) => {
            ...
            // Verificar si la hora tiene un formato válido (HH:MM) o (HH:MM:SS) - permitiendo horas > 23
            // Acepta HH:MM o HH:MM:SS, donde HH puede ser mayor a 23
            const regex = /^\d{2}:[0-5][0-9](:\d{2})?$/;
            
            if (regex.test(cleanHora)) {
              // Si tiene segundos, devolver solo HH:MM
              return cleanHora.substring(0, 5);
            }
            return '';
          };
