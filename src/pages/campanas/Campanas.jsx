...
const Campanas = () => {
        ...
        try {
            await Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    setCampanas(prev =>
                        prev.filter(campana => campana.id_campania !== id)
                    );
                    Swal.fire(
                        'Eliminado',
    ...
    const handleToggleEstado = async (campana) => {
        ...
        try {
            ...
            if (result.isConfirmed) {
                ...
                setCampanas(prev =>
                    prev.map(item =>
                        item.id_campania === campana.id_campania
    ...
    const filteredCampanas = campanas.filter(campana => {
        ...
        if (orderBy === 'years') {
            ...
            const yearB = parseInt(b.Anios?.years || 0);
            if (yearA < yearB) return orderDirection === 'asc' ? -1 : 1;
            if (yearA > yearB) return orderDirection === 'asc' ? 1 : -1;
            return 0;
        }
        return 0;
                        ...
                        <TableRow>
                            ...
                            <TableCell>
                                <TableSortLabel
                                    ...
                                    direction={orderBy === 'years' ? orderDirection : 'asc'}
                                    onClick={() => handleRequestSort('years')}
                                >
                                    Año
                                </TableSortLabel>
                            </TableCell>
