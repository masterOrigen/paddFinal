...
const prepareAlternativasForSave = (newOrderId, numeroCorrelativo) => {
  ...
  return alternativasToSave.map(alt => {
    ...
    useEffect(() => {
      if (alternativa && (alternativa.id || alternativa.calendar)) {
          ...
          if (alternativa.calendar) {
              try {
                  ...
                  setAlternativa(prev => {
                      ...
                      if (JSON.stringify(prev.cantidades) === JSON.stringify(calendarArray)) return prev;
                      return {
                          ...prev,
                  ...
                  // IMPORTANTE: Poblar cantidadesRef para que handleSave funcione correctamente
                  // Si no hacemos esto, al editar solo un día, los demás días se perderían al guardar
                  if (calendarArray && Array.isArray(calendarArray)) {
                      calendarArray.forEach(item => {
                          if (item.dia) {
                              cantidadesRef.current[item.dia] = item.cantidad;
                          }
                      });
                  }
    ...
    useEffect(() => {
      ...
      if (initialData) {
          // Inicializar con los datos proporcionados (sea nueva o existente editada)
          setAlternativa(initialData);
          
          // Obtener información del plan para el mes y año
          const targetOrdenId = initialData.id_orden || initialData.numerorden || ordenId;
          if (targetOrdenId) {
              fetchPlanInfo(targetOrdenId);
          }
          ...
          if (initialData.calendar) {
              try {
                  ...
                  // Populate cantidadesRef to ensure data persistence
                  if (calendarArray && calendarArray.length > 0) {
                      calendarArray.forEach(item => {
                          if (item.dia && item.cantidad) {
                              cantidadesRef.current[String(item.dia).padStart(2, '0')] = item.cantidad;
                          }
                      });
                  }
          ...
          // Force recalculate totals if we have initial data
          const calculatedData = calculateTotals(initialData);
          setAlternativa(calculatedData);

      } else if (alternativaId) {
          // Obtener datos de alternativa existente
          fetchAlternativa();
      }
  }, [alternativaId, isCreatingNew, initialData, ordenId]);
    ...
    const fetchPlanInfo = async (targetId) => {
      try {
          const idToUse = targetId || (alternativa ? alternativa.id_orden : ordenId);
          
          if (!idToUse) {
              console.log('No se pudo determinar el ID de la orden para cargar info del plan');
              return;
          }

          console.log('Fetching plan info for orden ID:', idToUse);
          ...
          const { data: ordenData, error: ordenError } = await supabase
              ...
              `)
              .eq('id_ordenes_de_comprar', idToUse)
          ...
          if (!ordenData) {
            console.log('No se encontró la orden con ID:', idToUse);
            return;
          }
    ...
    try {
        ...
        if (alternativaId && alternativaId.toString().startsWith('temp_')) {
            ...
            if (tempAlternativa) {
                ...
                // Forzar el cálculo de totales al cargar
                const calculatedData = calculateTotals(tempAlternativa);
                setAlternativa(calculatedData);
        ...
        // Usar calculateTotals para mantener consistencia
        const dataWithCalculatedTotals = calculateTotals(data);
        
        setAlternativa(dataWithCalculatedTotals);
        ...
        if (initialData) {
            ...
            // Asegurarse de que cantidades esté poblado desde calendar si es necesario
            if (!initialData.cantidades && initialData.calendar) {
                try {
                    if (typeof initialData.calendar === 'string') {
                        initialData.cantidades = JSON.parse(initialData.calendar);
                    } else if (Array.isArray(initialData.calendar)) {
                        initialData.cantidades = initialData.calendar;
                    }
                } catch (e) {
                    console.error('Error parsing calendar for initialData fallback:', e);
                    initialData.cantidades = [];
                }
            }

            // Calcular totales también para initialData
            const dataWithCalculatedTotals = calculateTotals(initialData);
            setAlternativa(dataWithCalculatedTotals);
   ...
   // Update the handleSave function to properly handle boolean values
    const handleSave = async (data = null) => {
     try {
         ...
         const sourceData = data || alternativa;
         
         console.log('handleSave called with sourceData:', sourceData);
         console.log('cantidadesRef.current keys:', Object.keys(cantidadesRef.current));

         // Usar el estado calendarData para el calendario
       const calendarToSave = sourceData.cantidades || calendarData;
       const cantidadesArrayFromRef = Object.keys(cantidadesRef.current).length > 0
         ? Object.entries(cantidadesRef.current)
             ...
             .map(([d, c]) => ({ dia: String(d).padStart(2, '0'), cantidad: c }))
         : calendarToSave;
         
       // Ensure calendar is not empty if sourceData has quantities
       const finalCalendar = (cantidadesArrayFromRef && cantidadesArrayFromRef.length > 0) 
            ? cantidadesArrayFromRef 
            : (Array.isArray(sourceData.cantidades) && sourceData.cantidades.length > 0 
                ? sourceData.cantidades 
                : []);

       console.log('Final calendar to save:', finalCalendar);

         // Preparar los datos de la alternativa con el calendario
       let alternativaData = {
           ...sourceData,
           calendar: JSON.stringify(finalCalendar),
           cantidades: finalCalendar // Keep quantities for immediate UI update if needed, though we delete it below usually
       };
      ...
      alternativaData.multiplicar_valor = sourceData.multiplicar_valor_unitario ? 1 : 0;
  ...
  const calculateTotals = (data) => {
    ...
    const descuento = parseFloat(data.descuento_pl) || 0;
    const recargo = parseFloat(data.recargo_plan) || 0;
    const multiplicar = data.multiplicar_valor_unitario;
    
    // Calcular total de cantidades
    const totalCantidades = (data.cantidades || []).reduce((sum, item) => sum + (Number(item.cantidad) || 0), 0);

    const multiplicador = multiplicar ? (totalCantidades > 0 ? totalCantidades : 1) : 1;
    
    let totalBruto = 0;
    let totalNeto = 0;
    const tipoGeneracion = data.Contratos?.id_GeneraracionOrdenTipo || 1; // 1: Neto, 2: Bruto

    if (tipoGeneracion === 1) { // Neto
  ...
  const handleCantidadInput = (dia, valor) => {
    ...
    // Actualizar array de cantidades en el estado
    let newCantidades = [];
    
    // Si tenemos datos previos en cantidadesRef, usarlos para construir el array completo
    if (Object.keys(cantidadesRef.current).length > 0) {
        newCantidades = Object.entries(cantidadesRef.current)
            .filter(([, v]) => v !== '' && v !== null)
            // Fix: Store dia as string to ensure consistency with PDF generator and CalendarioAlternativa
            .map(([d, v]) => ({ dia: String(d).padStart(2, '0'), cantidad: v }));
    } else {
        // Fallback al estado anterior si cantidadesRef está vacío (raro)
        newCantidades = [...(alternativa.cantidades || [])];
        const idx = newCantidades.findIndex(c => c.dia === dia);
        if (idx >= 0) newCantidades[idx] = { dia: String(dia).padStart(2, '0'), cantidad: valor };
        else newCantidades.push({ dia: String(dia).padStart(2, '0'), cantidad: valor });
    }
    ...
    if (autoFillCantidades) {
      ...
      const diasMes = getDiasDelMes(anio, mes);
      
      diasMes.forEach(({ dia: d }) => {
        cantidadesRef.current[d] = valor;
      });
      
      // Reconstruir array completo con el valor rellenado
      newCantidades = diasMes.map(({ dia: d }) => ({ dia: String(d).padStart(2, '0'), cantidad: valor }));
    }
    
    setAlternativa(prev => {
        const updated = { ...prev, cantidades: newCantidades };
        return calculateTotals(updated);
    });
  };
  ...
  const handleActualizar = async () => {
    // Calcular totales actualizados
          const updatedAlternativa = calculateTotals(alternativa);
          setAlternativa(updatedAlternativa);
          
          // Llamar a handleSave para guardar los cambios y notificar al padre
          await handleSave(updatedAlternativa);
        };
